...
def generate_data(
    ...
    if cross_response:
        cross_prod = [
            np.outer(np.array([x, y, z]), np.array([a, b, c]))
            for x, y, z, a, b, c in zip(
                obs_df["user_f0"],
                obs_df["user_f1"],
                obs_df["user_fh"],
                obs_df["camp_f0"],
        ...
        iprod = [
            np.sum(np.multiply(np.random.random((3, 3)), cross_prod[i]))
            for i in range(len(cross_prod))
        ]
        obs_df["response"] = np.random.binomial(
            n=1, p=sigmoid(np.asarray(iprod), a=response_sig_a)
        )
    else:
        iprod = (
